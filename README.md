...
这是一个独立的本地自动化仓库，用来读取 Excel bug 清单，筛选分配给你的前端 bug，然后让配置的 AI CLI（Codex 或 Claude Code）在每个 bug 独立的 git worktree 中尝试分析、修复、测试，并在早上提供一个 Next 审批工作台。
...
每一行就是一个 bug。工具会提取问题描述、备注、一级分类、二级分类、状态、日期等字段，也会提取 `截图1` / `截图2` / `截图3` 单元格中的 WPS `DISPIMG` 图片，并把截图或截图路径传给配置的 AI CLI。
...
- `target_app_path`：允许 AI CLI 修改和审批的工程目录
- `verify_commands`：每条 bug 修完后要运行的检查命令
- `prompt_context_paths`：默认放进提示词的工程文件或目录
- `max_concurrency`：该工作区建议并发数

`prompt.fields` 控制哪些 Excel 字段进入 AI 提示词；`prompt.template` 是初始化提示词；`prompt.context_paths` 可以补充工程内重点文件或目录。审批台右侧可以直接调整并保存这些配置。

## 并发执行

定时任务和“立即执行一次”会按 `max_concurrency` 并发处理 Excel 命中的 bug。每条 bug 使用独立 worktree 和独立分支，因此可以并行跑；默认并发是 `2`，建议日常保持 `2-3`，不建议超过 `4`，因为 AI CLI、lint、build 会同时占用 CPU、内存和磁盘 IO。
...
使用 Codex 时，图片路径会作为 `codex exec --image` 传入；使用 Claude Code 时，截图所在目录会通过 `claude --add-dir` 授权，图片路径也会写入 prompt。文件路径会写入 prompt，让 AI 按路径读取上下文。

AI 执行日志会按 CLI 类型写到：

```text
logs/codex/<fix-branch>.log
logs/claude/<fix-branch>.log
```

审批台右侧日志面板会展示当前选中分支的日志，并随页面自动刷新。
...
自动化仓库本身不会再由 AI CLI 主动提交。除非你明确说“提交自动化仓库改动”，否则我只会保留工作区改动给你检查。
...
- AI CLI 子进程只在对应 bug 的 worktree 中运行
- Codex 使用 `exec --full-auto --cd <worktree>`；Claude Code 使用 `--print --permission-mode bypassPermissions`
- 自动化不会修改目标 monorepo 的 git hooks 或 git config
- AI CLI 子进程的 `PATH` 会临时插入一个本地 `git` 包装器，用来阻止 `git push`
...
- `logs/<cli-tool>/<fix-branch>.log`，例如 `logs/codex/...` 或 `logs/claude/...`
