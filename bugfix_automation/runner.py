...
def run_once(config: Config, dry_run: bool = False) -> tuple[Path, Path]:
    ...
    for bug in bugs:
        ...
        if dry_run:
            results.append(_result(bug, "dry-run", branch, "Matched filters; no worktree or Codex invocation."))
            continue
        results.append(process_bug(config, bug, branch))
    return write_reports(run_dir, results)


def process_bug(config: Config, bug: BugRecord, branch: str) -> dict[str, Any]:
    worktree_path: Path | None = None
    try:
        existing_path = worktree_path_for_branch(config.worktree_root, branch)
        if existing_path.exists():
            return _result(bug, "skipped", branch, f"Worktree already exists: {existing_path}")
        existing_branch_path = branch_worktree_path(config.target_repo, branch)
        if existing_branch_path is not None:
            return _result(bug, "skipped", branch, f"Branch already checked out at: {existing_branch_path}")
        if branch_exists(config.target_repo, branch):
            return _result(bug, "skipped", branch, "Branch already exists in target repository.")
        ...
        _run(codex_command(config.codex_bin, str(worktree_path), prompt), cwd=worktree_path, path_prefix=git_wrapper_dir)
        ...
        if not has_app_changes(worktree_path, config.target_app_path):
            return _result(bug, "no-change", branch, "Codex finished without local changes.")
        commit_all(worktree_path, _commit_message(bug))
        return _result(bug, "committed", branch, f"Committed locally in {worktree_path}.")
    except Exception as exc:
        detail = f"{type(exc).__name__}: {exc}"
        if worktree_path is not None:
            detail = f"{detail}; worktree={worktree_path}"
        return _result(bug, "failed", branch, detail)


def codex_command(codex_bin: str, worktree_path: str, prompt: str) -> list[str]:
    return [
        codex_bin,
        "exec",
        "--sandbox",
...
def _result(bug: BugRecord, status: str, branch: str, detail: str) -> dict[str, Any]:
    return {
