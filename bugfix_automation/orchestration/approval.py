...
from bugfix_automation.domain.capability_system import install_capabilities, render_capability_contract
...
from bugfix_automation.git.runner import run as _run
from bugfix_automation.orchestration.bug_runner import list_bugs
from bugfix_automation.domain.ai_cli import ai_cli_command
from bugfix_automation.orchestration.bug_runner import assert_scope_clean, codex_log_path, runtime_path_prefix
...
from bugfix_automation.domain.task_state import is_task_active, set_task_state, task_state
from bugfix_automation.git.worktree import changed_paths, commit_all, create_no_push_git_wrapper, symlink_node_modules, tracked_changed_files, write_worktree_exclude
...
def reject_fix(config: Config, branch: str) -> None:
    ...
    from bugfix_automation.orchestration.fix_validation import discard_preview_for_source_branch
...
def rework_fix(config: Config, branch: str, note: str = "", file_paths: list[str] | None = None, image_paths: list[str] | None = None) -> None:
    ...
    try:
        _run(ai_cli_command(config.cli_tool, str(fix.path), prompt, normalized_images), cwd=fix.path, path_prefix=path_prefix, stdin_text=prompt, log_path=codex_log_path(config, branch))
...
def _rework_prompt(config: Config, branch: str, note: str, file_paths: list[str], image_paths: list[Path]) -> str:
    from bugfix_automation.domain.prompt import PROMPTS_DIR
