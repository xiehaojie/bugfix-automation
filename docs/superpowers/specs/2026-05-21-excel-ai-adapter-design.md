...
- 将 Excel 适配、筛选、prompt、工作区等运行时可变配置保存到 SQLite，减少 `config.yaml` 的日常编辑压力。
...
- 第一版不删除 `config.yaml`。它仍保留启动路径、端口、数据库位置和 CLI 工具等核心引导配置。
...
- `config.yaml` 只保留启动前必须知道的核心字段，例如 `approval_web_port`、`approval_api_port`、`storage_db_path`、`data_root`、`logs_root`、`runs_root`、`cli_tool`、`launchd_label`。
- SQLite 保存运行时可变配置，例如当前 Excel 文件、工作区、筛选规则、prompt 字段、Excel 专用 prompt、字段映射、并发数和定时任务时间。
- 环境变量仍保留最高优先级，适合临时覆盖本机路径和端口。
- 读取配置时按 `环境变量 > SQLite 当前配置 > config.yaml > 代码默认值` 合并。

SQLite 中保存的 Excel profile 逻辑结构示例：
...
```json
{
  "excel_profile": {
    ...
    "prompt": {
      "fields": ["问题描述", "备注", "备注2"],
      "template": "请优先根据当前 Excel 的业务语义修复前端可独立完成的问题。",
      "branch_summary_fields": ["问题描述", "一级分类"]
    }
  }
}
```

`config.yaml` 第一版建议收敛为：
...
迁移期允许 `config.yaml` 继续包含旧字段。SQLite 没有对应配置时，旧字段仍作为回退值读取。

如果需要人工复制环境，系统提供“导出当前配置”能力，把 SQLite 当前配置渲染成可读 YAML 快照。这个导出文件用于备份和迁移，不作为日常主配置。

旧版 YAML 中的 Excel profile 示例仍可被读取为回退配置：
...
8. 前端调用 `/api/excel/adapter/save`，后端写入 SQLite 当前配置，并创建配置快照。
...
- `excel`: 当前 Excel 路径、sheet 名、已处理状态列和值。
- `workspaces`: 工作区列表和当前 active workspace。
- `automation`: 并发数、定时任务时间、CLI 工具。
- `filters`: 当前筛选规则。
- `prompt`: 当前 prompt 字段、模板、上下文路径、分支摘要字段。
- `excel_profile`: AI 识别并由用户确认的字段映射和 Excel 专用 prompt。

`load_config` 从 YAML 读取 bootstrap，再读取 SQLite 设置并合并。`update_config_yaml` 不再作为审批台保存配置的主要出口，后续可保留给 bootstrap 字段和兼容旧调用。
...
AI 调用使用现有 `config.cli_tool`，超时时间默认 120 秒。失败时返回可读错误，不修改 SQLite 当前配置。
...
- SQLite 不可写：保存失败并提示数据库路径；不回退写入 YAML，避免配置源分裂。

## 测试策略

- 配置解析测试：读取 `excel_profile`，缺失时使用默认映射。
- 配置存储测试：`load_config` 按 `环境变量 > SQLite > YAML > 默认值` 合并。
- 配置迁移测试：旧 YAML 中的 `filters`、`prompt`、`workspaces` 可以导入 SQLite 当前配置。
...
现有 `config.yaml` 不需要立即修改。迁移期会提供一次性导入：

1. 启动时如果 SQLite 没有当前配置，系统从 `config.yaml` 读取旧字段并写入 `app_settings`。
2. 导入完成后，后续审批台保存配置只写 SQLite。
3. `config.yaml` 中的旧字段继续作为回退，不主动删除，避免破坏用户本地配置。
...
- `excel_profile.prompt.fields` 同步写入 SQLite 的 `prompt.fields`，让旧 UI 继续展示一致结果。
- `excel_profile.prompt.template` 同步写入 SQLite 的 `prompt.template`，避免两套 prompt 配置混淆。
- `excel_profile.prompt.branch_summary_fields` 同步写入 SQLite 的 `prompt.branch_summary_fields`。

第一版采用 SQLite 内部同步写入，减少配置入口数量。
...
`config.yaml` 不再是完整运行状态的唯一来源。需要审计或复现时，以 `config_snapshots` 和 `app_settings` 为准；需要跨机器迁移时，使用导出的配置快照。
...
- AI 识别失败不会改写 SQLite 当前配置或 `config.yaml`。
- 审批台保存筛选、prompt、工作区和 Excel 适配时写入 SQLite，不再日常改写 `config.yaml`。
