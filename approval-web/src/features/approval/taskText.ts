...
export function taskHeadline(item: TaskLike) {
  if (item.active) return "AI 正在执行";
  if (item.task_status === "pending-approval") return "AI 已完成，等待你审批";
  if (item.task_status === "failed") return "执行失败，需要重新触发";
  if (item.task_status === "no-change") return "AI 已结束，没有产出前端改动";
...
export function taskSubtext(item: TaskLike) {
  ...
  if (item.task_status === "failed") return item.task_detail || "请打开 AI 日志，查看失败原因。";
  if (item.task_status === "no-change") return item.task_detail || "AI 跑完了，但没有生成可提交的前端改动。";
...
export function phaseLabel(phase: string) {
  ...
  if (phase === "codex") return "AI 正在分析并修改";
